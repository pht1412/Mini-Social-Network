      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {requests.map(u => (
          <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px', borderRadius: '8px', transition: '0.2s' }}>
            ...
            />
            <div style={{flex: 1}}>
              <div style={{ fontWeight: 'bold', fontSize: '14px', color: '#050505' }}>{u.fullName}</div>
              <div style={{ fontSize: '12px', color: '#65676b', marginBottom: '5px' }}>{u.studentCode}</div>
